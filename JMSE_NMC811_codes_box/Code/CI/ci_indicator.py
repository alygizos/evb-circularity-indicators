...
import pandas as pd
# compute_ci_cullen inlined from the shared "Code Shared AL/indicators.py" so this
# script is self-contained (no external indicators.py needed to run standalone).
def compute_ci_cullen(alpha=None, beta=None,
                      recovered_eol=None, total_demand=None,
                      energy_recovery=None, energy_primary=None):
    """
    Circularity Index (Cullen, 2017): CI = alpha * beta, clipped to [0, 1].
      alpha = recovered EOL material / total material demand
      beta  = 1 - (energy to recover / energy for primary production)
...
def plot_ci_with_error_bars(csv_path='ci_parameters.csv', n_samples=1000,
    ...
    # Prepare data
    categories = ['Steel', 'Aluminum', 'Cell', 'Final CI']
    ...
    colors = [
        '#762a83',  # Steel – purple
        '#9970ab',  # Aluminum – teal blue
        '#c2a5cf',  # Cell – green
        '#e7d4e8'   # Final CI – light green
        ]

    # Create bar plot with error bars
    x_pos = np.arange(len(categories))
    bars = ax.bar(x_pos, means, color=colors, edgecolor='black',
                   linewidth=1.5, alpha=0.7, width=0.4)
...
def tornado_plot_ci(component='steel', csv_path='ci_parameters.csv',
    ...
    bars_low = ax.barh(y_pos, low_impacts, height=0.4, align='center',
                       color='#dfc27d', alpha=0.8, label=f'-{variation_pct*100}%')
    bars_high = ax.barh(y_pos, high_impacts, height=0.4, align='center',
                        color='#80cdc1', alpha=0.8, label=f'+{variation_pct*100}%')
    ...
    # Customize
    ax.set_yticks(y_pos)
    ax.set_yticklabels(param_labels, fontweight='bold')
    ax.set_xlabel('Change in CI', fontsize=13)
...
if __name__ == "__main__":
    ...
    try:
        ...
        # Save cumulative CI results to common CSV (copied from the shared LFP
        # ci_indicator.py so CI writes its row like PCI/CEI/ECPI/MRE).
