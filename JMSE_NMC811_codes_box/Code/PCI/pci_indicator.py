...
def monte_carlo_pci(csv_file='pci_material_parameters.csv', n_samples=1000,
...
def plot_pci_with_error_bars(csv_file='pci_material_parameters.csv', n_samples=1000,
    ...
    num_materials = len(materials) - 1
    colors = list(sns.color_palette("viridis", num_materials))
    colors.append('#e74c3c')  # Red color for final PCI
    ...
    label_names = [m if m != 'FINAL_PCI' else 'Total\n(Weighted)' for m in materials]
    ax.set_xticks(x_pos)
...
def sensitivity_analysis(csv_file='pci_material_parameters.csv', material_name=None,
    ...
    bars1 = ax1.bar(x_pos, Si['S1'], yerr=Si['S1_conf'], color=colors,
                    edgecolor='black', linewidth=1.2, capsize=5, alpha=0.8)
    ax1.set_xlabel('Parameter', fontweight='bold', fontsize=12)
    ax1.set_ylabel('First-order Sobol Index (S1)', fontweight='bold', fontsize=12)
    ax1.set_title(f'First-order Sensitivity - {material_name}', fontweight='bold', fontsize=13)
    ...
    # Total-order indices
    bars2 = ax2.bar(x_pos, Si['ST'], yerr=Si['ST_conf'], color=colors,
                    edgecolor='black', linewidth=1.2, capsize=5, alpha=0.8)
    ax2.set_xlabel('Parameter', fontweight='bold', fontsize=12)
    ax2.set_ylabel('Total-order Sobol Index (ST)', fontweight='bold', fontsize=12)
    ax2.set_title(f'Total-order Sensitivity - {material_name}', fontweight='bold', fontsize=13)
...
def tornado_plot(csv_file='pci_material_parameters.csv', material_name=None,
                 variation_pct=0.05, save_path='pci_tornado.png', dpi=300, figsize=(10, 8)):
    """
    ...
    ax.set_xlabel('Change in PCI', fontweight='bold', fontsize=14)
